...
int main(void)
{
	...
	char input_am_pm_1, input_am_pm_2;
	...
	printf("Enter a 12-hour time (append am/pm): ");
	scanf("%d:%d%c%c", &input_hours, &input_minutes, &input_am_pm_1,
	      &input_am_pm_2);
	input_am_pm_1 = toupper(input_am_pm_1);
	input_am_pm_2 = toupper(input_am_pm_2);

	if (input_am_pm_1 == 'A' || input_am_pm_2 == 'A') {
		if (input_hours == 12) {
			input_hours = 0;
		}
	} else if (input_am_pm_1 == 'P' || input_am_pm_2 == 'P') {
		input_hours += 12;
	}
